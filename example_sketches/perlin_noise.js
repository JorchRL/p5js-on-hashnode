// Taken from: https://openprocessing.org/sketch/566877
//-------------------------------
...
function draw() {
  ...
  for (var i = 0; i < nums; i++) {
    ...
    fade_ratio = min(
      ((maxLife - particles[i].life) * 5) / maxLife,
      fade_ratio
    );

    fill(
      red(particle_color),
      green(particle_color),
      blue(particle_color),
      alpha * fade_ratio
    );
...
function Particle() {
  ...
  this.move = function (iterations) {
    if ((this.life -= 0.01666) < 0) this.respawnTop();
    while (iterations > 0) {
      var transition = map(
        this.pos.x,
        padding_side,
